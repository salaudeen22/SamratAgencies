    ...
    >
      <div className="flex justify-between items-start mb-3 gap-2">
        <div className="flex-1">
          <div className="flex items-center gap-2 mb-1">
            <div className="p-1.5 rounded-lg" style={{ backgroundColor: '#E6CDB1' }}>
              <HiShoppingBag className="w-4 h-4" style={{ color: '#816047' }} />
            </div>
            <p className="text-sm font-mono font-semibold" style={{ color: '#2F1A0F' }}>
              #{order._id.slice(-8)}
            </p>
          </div>
      ...
      <div className="space-y-1.5 mb-3 py-2 border-y" style={{ borderColor: '#f3f4f6' }}>
        {order.items?.slice(0, 1).map((item, index) => (
          <p key={index} className="text-sm truncate font-medium" style={{ color: '#2F1A0F' }}>
      ...
      <div className="flex justify-between items-center">
        <span className="text-xs font-semibold uppercase tracking-wide" style={{ color: 'rgba(129, 96, 71, 0.6)' }}>Total Amount</span>
        <span className="text-lg font-bold" style={{ color: '#816047' }}>
          ₹{(order.totalPrice || order.totalAmount || 0).toLocaleString()}
        </span>
      </div>

      <div className="mt-3 pt-3 border-t flex items-center justify-center gap-2 text-sm font-medium group-hover:gap-3 transition-all" style={{ borderColor: '#f3f4f6', color: '#816047' }}>
