...
const ProductDetail = () => {
  ...
  return (
    <>
      <SEO
        title={`${product.name} - ${product.discount > 0 ? `${product.discountType === 'percentage' ? product.discount + '% OFF' : '₹' + product.discount + ' OFF'} - ` : ''}Starting at ₹${product.discount > 0 ? Math.round(product.discountType === 'percentage' ? product.price - (product.price * product.discount / 100) : product.price - product.discount).toLocaleString() : product.price.toLocaleString()} | Samrat Agencies`}
        description={
          product.description
            ? `${product.description.slice(0, 140)}... Price: ${product.discount > 0 ? 'SALE ' + (product.discountType === 'percentage' ? product.discount + '% OFF' : '₹' + product.discount + ' OFF') + ' - Now ₹' + Math.round(product.discountType === 'percentage' ? product.price - (product.price * product.discount / 100) : product.price - product.discount).toLocaleString() : '₹' + product.price.toLocaleString()}. ${product.availabilityType === 'immediate' ? 'Same-Day Delivery Available' : 'Ready in ' + product.deliveryDays + ' Days'}. ${product.rating > 0 ? product.rating.toFixed(1) + '/5' : '5.0/5'} Customer Rating. FREE Home Delivery in Bangalore, Chennai, Hyderabad. No Cost EMI Available - Bajaj Finserv, HDFC, ICICI. ${product.specifications ? Object.values(product.specifications).slice(0, 2).join(', ') + '.' : 'Premium Quality.'} Warranty Included. Trusted Since 1996.`
            : `Shop ${product.name} at Best Prices in Bangalore. ${product.discount > 0 ? (product.discountType === 'percentage' ? product.discount + '% OFF' : '₹' + product.discount + ' OFF') + ' - ' : ''}Starting ₹${product.discount > 0 ? Math.round(product.discountType === 'percentage' ? product.price - (product.price * product.discount / 100) : product.price - product.discount).toLocaleString() : product.price.toLocaleString()}. ${product.availabilityType === 'immediate' ? 'Same-Day Delivery Available' : 'Ready in ' + product.deliveryDays + ' Days'}. ${product.rating > 0 ? product.rating.toFixed(1) : '5.0'}/5 Customer Rating. ${product.category ? 'Best ' + (typeof product.category === 'object' ? product.category.name : product.category) + ' in South India' : 'Premium Quality Furniture'}. FREE Delivery across Bangalore, Chennai, Hyderabad, Kochi. No Cost EMI - Bajaj Finserv, HDFC, ICICI, SBI. ${product.specifications ? Object.values(product.specifications).slice(0, 2).join(', ') + '.' : 'Quality Assured.'} Warranty Included. Trusted Since 1996.`
        }
